...
def fit_stan_mrp_and_causal(model_variant, imputed = False, imp_i = None, fit_path = None, num_chains = 4, iter_warmup = 1000, iter_sampling = 500, seed = 200, adapt_delta = None, include_enp = False, thin = 5):
    # data
    ...
    if model_variant == 'mrp_ord':
        if include_enp:
            model = CmdStanModel(stan_file="./stan_codes/mrp_ord_uk_w_enp.stan",cpp_options={'STAN_THREADS': True})
        else:
    ...
    if fit_path is None:
        if model_variant == 'mrp_ord':
            fit_path = f'./idata/uk/rsv_intent_ord_soc{enp_str}.nc'
        elif model_variant == 'mrp_ber':
            fit_path = f'./idata/uk/rsv_intent_ber_soc{enp_str}.nc'
        elif model_variant == 'rsv_causal_wo_soc':
            fit_path = './idata/uk/rsv_intent_causal_wo_soc.nc'
        elif model_variant == 'rsv_causal_w_soc':
            fit_path = f'./idata/uk/rsv_intent_causal_w_soc{enp_str}.nc'
    if imputed:
        _fit_path_imp = fit_path[:-3]
        fit_path_imp = f'{_fit_path_imp}_imp{imp_i}{enp_str}.nc'
        fit_path_imp_comb = f'{_fit_path_imp}_imp_combined{enp_str}.nc' 
        idata.to_netcdf( fit_path_imp, overwrite_existing = True )
        if imp_i == 1:
            idata.to_netcdf( fit_path_imp_comb, overwrite_existing = True )
        else:
            idata_combined = az.from_netcdf( fit_path_imp_comb )
            idata_combined = az.concat(idata_combined, idata.copy(), dim = 'chain')
            idata_combined.to_netcdf( fit_path_imp_comb, overwrite_existing = True )

    else:
        idata.to_netcdf(fit_path, overwrite_existing = True)
...
def fit_stan_child(model_variant, imputed = False, imp_i = None, fit_path = None, num_chains = 4, iter_warmup = 1000, iter_sampling = 500,
                    seed = 200, adapt_delta = None, include_enp = False, thin = 5, reduce_to_4_ages = True):
    survey = pd.read_csv('./dat/trs_questionnaire/uk/survey_coarsened.tsv', sep ='\t', index_col = 0)
    survey_ch_1 = survey[~survey['X1_rsv_ch_i'].isna()].copy()

    survey_ch_1 = survey[~survey['X1_rsv_ch_i'].isna()].copy()
    vars_needed = ['rsv_intent_hyp_pre', 'rsv_intent_hyp_post', 'RCT', 'X1_rsv_ch_i', 'X1_relationship_ch_i', 'X1_sex_ch_i', 'X1_permission_ch_i']
    soc_dems_original = ['AGE_uk', 'SEX_uk', 'EDU_uk', 'REL_uk', 'ETH_uk', 'LAN_uk', 'MIG_uk', 'BUK_uk', 'ENP_uk', 'DIS_uk', 'HEA_uk', 'EMP_uk', 'SCH_uk']
    soc_dems_rename = {k : k.strip('_uk') for k in soc_dems_original}
    soc_dems = list(soc_dems_rename.values())

    survey_ch_1 = survey_ch_1[ vars_needed + soc_dems_original]
    survey_ch_1 = survey_ch_1.rename(soc_dems_rename, axis = 1)
    vars_needed = vars_needed + soc_dems
    survey_ch_1 = survey_ch_1.dropna()

    for var in survey_ch_1.columns:
        _df = pd.Series(survey_ch_1[var].value_counts())
        print(f'Variable {var}:')
        print(_df)

    reduce_dims_dict = {}
    for c in soc_dems:
        _unique = survey_ch_1[c].unique()
        reduce_dims_dict[c] = dict(zip(_unique, _unique))

    if reduce_to_4_ages:
        reduce_dims_dict['AGE']['45-54'] = '45+'
        reduce_dims_dict['AGE']['55-64'] = '45+'
        reduce_dims_dict['AGE']['65+'] = '45+'
    ...
    reduce_dims_dict['LAN']['Polish'] = 'Other'

    reduce_dims_dict['HEA']['Very bad'] = 'Bad or very bad'
    reduce_dims_dict['HEA']['Bad'] = 'Bad or very bad'

    reduce_dims_dict['EMP']['Economically inactive: Retired'] = 'Economically inactive: Other'
    reduce_dims_dict['EMP']['Economically inactive: Long-term sick or disabled'] = 'Economically inactive: Other'

    reduce_dims_dict['EMP']['Economically inactive: Student'] = 'Student'
    reduce_dims_dict['EMP']['Economically Active Full-time students'] = 'Student'

    reduce_dims_dict['X1_relationship_ch_i'] = {'Mother' : 'Parent', 'Father' : 'Parent', 'Uncle or aunt' : 'Other', 'Grandparent' : 'Other', 'Brother or sister' : 'Other', 'Other (please state)' : 'Other' }

    reduce_dims_dict['X1_sex_ch_i'] = {'Female' : 'Female', 'Male' : 'Male', 'Intersex / Other' : np.nan}

    reduce_dims_dict['X1_permission_ch_i'] = {'Yes' : 'Yes', 'No' : 'No'}

    for c in soc_dems + ['X1_relationship_ch_i','X1_sex_ch_i', 'X1_permission_ch_i']:
            survey_ch_1[c] = survey_ch_1[c].map(reduce_dims_dict[c])

    print( survey_ch_1[survey_ch_1.isna().sum(axis = 1)>0] )
    ...
    recode_mapping['rsv_intent_hyp_pre'] = {'No, definitely not':1, 'Unsure, but leaning towards no':2,'Unsure, but leaning towards yes':3, 'Yes, definitely':4}
    recode_mapping['rsv_intent_hyp_post'] = {'No, definitely not':1, 'Unsure, but leaning towards no':2,'Unsure, but leaning towards yes':3, 'Yes, definitely':4}
    recode_mapping['X1_rsv_ch_i'] = {'No, definitely not':1, 'Unsure, but leaning towards no':2,'Unsure, but leaning towards yes':3, 'Yes, definitely':4}
    recode_mapping['RCT'] = {'RCT0':1, 'RCT1':2, 'RCT2':3, 'RCT3':4, 'RCT4':5, 'RCT5':6, 'RCT6':7}

    survey_recode = survey_ch_1.copy()
    print(f'list of sociodems is now: {soc_dems}')
    for c in soc_dems + ['X1_relationship_ch_i','X1_sex_ch_i', 'X1_permission_ch_i']:
        # try:
    ...
    survey_recode['rsv_intent_hyp_pre'] = survey_recode['rsv_intent_hyp_pre'].map( recode_mapping['rsv_intent_hyp_pre'] )
    survey_recode['rsv_intent_hyp_post'] = survey_recode['rsv_intent_hyp_post'].map( recode_mapping['rsv_intent_hyp_post'] )
    survey_recode['X1_rsv_ch_i'] = survey_recode['X1_rsv_ch_i'].map( recode_mapping['X1_rsv_ch_i'] )
    survey_recode['RCT'] = survey_recode['RCT'].map( recode_mapping['RCT'] )
    ...
    'N' : survey_recode.shape[0],
    'N_dep' : 4,
    'N_age' : len( survey_recode['AGE_recode'].unique() ),
    'N_rel' : len( survey_recode['REL_recode'].unique() ),
    'N_eth' : len( survey_recode['ETH_recode'].unique() ),
    'N_lan' : len( survey_recode['LAN_recode'].unique() ),
    'N_emp' : len( survey_recode['EMP_recode'].unique() ),
    'N_edu' : len( survey_recode['EDU_recode'].unique() ),
    'N_dis' : len( survey_recode['DIS_recode'].unique() ),
    'N_hea' : len( survey_recode['HEA_recode'].unique() ),
    # 'N_yuk' : len( survey_recode['YUK'].unique() ),
    # 'N_sch' : len( survey_recode['SCH'].unique() ),
    # 'N_buk' : len( survey_recode['BUK'].unique() ),
    # 'N_mig' : len( survey_recode['MIG'].unique() ),
    # 'N_hhlan' : len( survey_recode['HHLAN'].unique() ),
    # 'N_sex' : len( survey_recode['SEX'].unique() ),
    # 'N_reg' : len( survey_recode['REG'].unique() ),
    'age' : survey_recode['AGE_recode'] + 1,
    'rel' : survey_recode['REL_recode'] + 1,
    'eth' : survey_recode['ETH_recode'] + 1,
    'lan' : survey_recode['LAN_recode'] + 1,
    ...
    'edu' : survey_recode['EDU_recode'] + 1,
    'sex' : survey_recode['SEX_recode'] + 1,
    # 'reg' : survey_recode['REG_recode'] + 1,
    'dis' : survey_recode['DIS_recode'] + 1,
    'hea' : survey_recode['HEA_recode'] + 1,
    # 'yuk' : survey_recode['YUK_recode'] + 1,
    # 'sch' : survey_recode['SCH_recode'] + 1,
    'buk' : survey_recode['BUK_recode'] + 1,
    'mig' : survey_recode['MIG_recode'] + 1,
    # 'hhlan' : survey_recode['HHLAN_recode'],
    'N_treat' : 7,
    'y_child' : survey_recode['X1_rsv_ch_i'],
    'y_post' : survey_recode['rsv_intent_hyp_post'],
    'y_pre' : survey_recode['rsv_intent_hyp_pre'], 
    'treat' : survey_recode['RCT'],
    #'N_rel_ch_i' : survey_recode[''],
        # 'rel_ch_i' : survey_recode[''],
        # 'sex_ch_i' : survey_recode[''],
        # 'per_ch_i' : survey_recode[''],
        'rel_ch_i' : survey_recode['X1_relationship_ch_i_recode'] + 1,
        'sex_ch_i' : survey_recode['X1_sex_ch_i_recode'] + 1,
        'per_ch_i' : survey_recode['X1_permission_ch_i_recode'] + 1,
    }

    if model_variant == 'child_cond_on_post':
        model = CmdStanModel(stan_file="./stan_codes/ord_causal_child_soc_cond_on_post.stan", cpp_options={'STAN_THREADS': True})
    elif model_variant == 'child_cond_on_pre':
        model = CmdStanModel(stan_file="./stan_codes/ord_causal_child_soc_cond_on_pre.stan", cpp_options={'STAN_THREADS': True})
    elif model_variant == 'child_cond_on_pre_and_post':
        model = CmdStanModel(stan_file="./stan_codes/ord_causal_child_soc_cond_on_pre_and_post.stan", cpp_options={'STAN_THREADS': True})
    else:
        raise  ValueError('non valid model variant')
    ...
    fit_path = f'./idata/uk/{model_variant}.nc'
    ...
    if not imputed:
        survey_ch_1.to_csv(f'./dat/dat_for_inference/uk/survey_{model_variant}.tsv', sep = '\t')
        survey_recode.to_csv(f'./dat/dat_for_inference/uk/survey_recode_{model_variant}.tsv', sep = '\t')
    with open(f'./dat/dat_for_inference/uk/data_dict_{model_variant}.p', 'wb') as f:
        pickle.dump(data, f)
    with open(f'./dat/dat_for_inference/uk/var_mappings_{model_variant}.p', 'wb') as f:
...
def fit_stan(model_variant, imputed=False, imp_i=None, fit_path=None, num_chains=4, iter_warmup=1000, iter_sampling=500, seed=200, adapt_delta=None, include_enp=False, thin = 5):
    # Call fit_stan_child() for specific model_variant values
    if model_variant in ['child_cond_on_post', 'child_cond_on_pre', 'child_cond_on_pre_and_post']:
        fit_stan_child(model_variant, imputed=imputed, imp_i=imp_i, fit_path=fit_path, num_chains=num_chains, iter_warmup=iter_warmup, iter_sampling=iter_sampling, seed=seed, adapt_delta=adapt_delta, include_enp=include_enp, thin = thin)
    else:
        # Call fit_stan_mrp_and_causal() for other model_variant values
        fit_stan_mrp_and_causal(model_variant, imputed=imputed, imp_i=imp_i, fit_path=fit_path, num_chains=num_chains, iter_warmup=iter_warmup, iter_sampling=iter_sampling, seed=seed, adapt_delta=adapt_delta, include_enp=include_enp, thin = thin)
