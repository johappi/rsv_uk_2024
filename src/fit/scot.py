...
def fit_stan(model_variant, imputed = False, imp_i = None, fit_path = None, num_chains = 4, iter_warmup = 1000, iter_sampling = 500, seed = 200, adapt_delta = None):
    ...
    if not imputed:
        survey = pd.read_csv('./dat/trs_questionnaire/scotland/survey_scot.tsv', sep ='\t', index_col = 0)
    else:
        survey = pd.read_csv(f'./dat/trs_questionnaire/scotland/survey_scot_imp{imp_i}.tsv', sep ='\t', index_col = 0)
    ...
    micro_itl3 = pd.read_csv(f'./dat/trs_microdata/scotland/itl3_renamed.tsv', sep ='\t', index_col = 0)
    ...
    reduce_dims_dict['LAN_scot']['Portuguese'] = 'Other'

    reduce_dims_dict['ETH_scot']['White: Gypsy or Irish Traveller'] = 'White: Other White'
    ...
    # reduce_dims_dict['ENP_scot']['Not at all'] = 'Well or not well or not at all'
    # reduce_dims_dict['ENP_scot']['Not well'] = 'Well or not well or not at all'
    # reduce_dims_dict['ENP_scot']['Well'] = 'Well or not well or not at all'
    reduce_dims_dict['ENP_scot']['Not at all'] = 'Not well or not at all'
    reduce_dims_dict['ENP_scot']['Not well'] = 'Not well or not at all'
    ...
    survey = survey[socio_dems_original + ['rsv_intent_hyp_pre','rsv_intent_hyp_post','RCT']].copy()

    micro_itl3 = micro_itl3.groupby( [ k for k in micro_itl3.columns if k not in ['n'] ], as_index = False )['n'].sum()

    # rename columns
    socio_dems_recode = { k : k.strip('_scot') for k in socio_dems_original }
    socio_dems_recode = { k : k.strip('_v2') for k in socio_dems_recode }
