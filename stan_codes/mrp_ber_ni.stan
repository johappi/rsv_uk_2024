...
model {
  ...
  vector[N] lin_pred = alpha + beta_age[age] + [beta_sex, -beta_sex][sex]' + beta_edu[edu] + beta_eth[eth] + beta_lan[lan] + beta_rel[rel] + beta_emp[emp] + beta_reg[reg]
                       + beta_enp[enp] + beta_dis[dis] + beta_hea[hea]
                       + [beta_sch,-beta_sch][sch]' + [beta_buk,-beta_buk][buk]' + [beta_hhlan,-beta_hhlan][hhlan]';
...
// beta_yuk_raw2 ~ normal(0, 1);
  ...
  { sigma_age, sigma_edu, sigma_eth, sigma_lan, sigma_rel, sigma_emp, sigma_reg, sigma_enp, sigma_dis, sigma_hea} ~ normal(0, 1);
}
...
vector[N_pred] lin_pred = alpha + beta_age[age_pred] + [beta_sex, -beta_sex][sex_pred]' + beta_edu[edu_pred] + beta_eth[eth_pred] + beta_lan[lan_pred] + beta_rel[rel_pred] + beta_emp[emp_pred] + beta_reg[reg_pred]
                       + beta_enp[enp_pred] + beta_dis[dis_pred] + beta_hea[hea_pred]
                       + [beta_sch,-beta_sch][sch_pred]' + [beta_buk,-beta_buk][buk_pred]' + [beta_hhlan,-beta_hhlan][hhlan_pred]';
