...
// pre effects
c = c_loc + ordered_pred( c_tilde, c_scale );
...
for (n in 1:N){
	// y_child[n] ~ ordered_logistic( beta_child * sum( theta_child[ : y_pre[n] - 1] ) + lin_pred_child[n], c_child  ); 
	y_child[n] ~ ordered_logistic( lin_pred_child[n], c_child[y_pre[n]]  ); 
	y_pre[n] ~ ordered_logistic( lin_pred[n], c );
	}
...
for (n in 1:N){
	...
	for (k_pre in 1:N_dep) {
		y_log_prob_pre[n,k_pre] = ordered_logistic_lpmf(k_pre | lin_pred[n], c );
		for (k_child in 1:N_dep){
			y_log_prob_child[n, k_pre, k_child] = ordered_logistic_lpmf( k_child | lin_pred_child[n],  c_child[k_pre] );
		}
	}
}
